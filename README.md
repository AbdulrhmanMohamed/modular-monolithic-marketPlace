...
#### Step 1: Extract First Service (Products)

```
1. Move Products.csproj → services/products/
2. Add Program.cs with:
   var builder = WebApplication.CreateBuilder(args);
   builder.Services.AddControllers();
   builder.Services.AddMediatR(...);
   // ...
3. Deploy as container
4. Update Host to call via HTTP:
   var response = await httpClient.PostAsJsonAsync(
       "http://products-service/api/product", command);
```

#### Extraction Checklist

- [ ] Module has its own DbContext (✅ Yes)
- [ ] No direct dependencies on other modules (✅ Yes, via MediatR)
- [ ] Events for cross-module communication (⚠️ To add: use message bus)
- [ ] Independent deployment pipeline (❌ To create)
...
```
GitHub Actions → Docker Build → Container Registry → K8s/Azure App Service
```
...
COPY . .
RUN dotnet restore
RUN dotnet publish src/Host -c Release -o /app
